...
public class ConsumerTask implements Runnable {
    ...
    public ConsumerTask(KafkaStream<byte[], byte[]> a_stream, int a_threadNumber,String a_tag, Set<String> a_whiteList, String a_outPath) {
        ...
        for(String type : m_whiteList){
        	HdfsWriter hw = new HdfsWriter(m_outPath, type, m_tag, m_threadNumber);
        	writer.put(type, hw);
        }
    ...
    public void run() { 	
		...
		//HdfsWriter abc = new HdfsWriter("/mnt", "playlog", m_tag, m_threadNumber);
		try {
			while (it.hasNext()) {
				...
				if (out != null) {
					if (m_whiteList.contains(out[0])) {
						...
						hw.writePeriod(
								"Thread " + m_tag + " " + m_threadNumber + ": " + "[" + out[0] + "]" + out[1]);
					}
				}
			}
			//abc.clean();
